...
export default function ClosetScreen() {
  ...
  const pickImage = async () => {
    Alert.alert('Add clothing item', 'Choose a photo source', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          if (!result.canceled) {
            ...
            setLabel(''); setBrand(''); setSelectedColors([]); setSelectedCategory('Tops');
        ...
        <View style={styles.detailOverlay}>
          <View style={styles.detailSheet}>
            ...
            <View style={styles.detailInfo}>
              ...
              {selectedItem?.colors?.length ? <Text style={styles.detailInfoText}>Colors: {selectedItem.colors.join(', ')}</Text> : null}
              <Text style={styles.detailInfoText}>
                Status: {selectedItem && dirtyIds.includes(selectedItem.id) ? 'Dirty 🧺' : 'Clean ✓'}
              </Text>
              <Text style={styles.detailInfoText}>Number of Wears: {selectedItem?.wornCount}</Text>
            </View>
            <View style={styles.detailBtns}>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  Alert.alert('Move to closet', 'Which closet?', [
                    { text: 'Cancel', style: 'cancel' },
                    ...closets.map(c => ({
                      text: `${c.emoji} ${c.name}`,
                      onPress: () => {
        ...
        <View style={styles.overlay}>
          <ScrollView scrollEnabled={false} keyboardShouldPersistTaps="handled">
            <View style={styles.sheet}>
              ...
              <TextInput style={styles.input} placeholder="Item name" placeholderTextColor="#bbb" value={editLabel} onChangeText={setEditLabel} />
              <TextInput style={styles.input} placeholder="Brand — optional" placeholderTextColor="#bbb" value={editBrand} onChangeText={setEditBrand} />
        ...
        <View style={styles.overlay}>
          <ScrollView scrollEnabled={false} keyboardShouldPersistTaps="handled">
            <View style={styles.sheet}>
              ...
              <TextInput style={styles.input} placeholder="e.g. White linen shirt" placeholderTextColor="#bbb" value={label} onChangeText={setLabel} />
              <TextInput style={styles.input} placeholder="Brand (e.g. Zara, Nike) — optional" placeholderTextColor="#bbb" value={brand} onChangeText={setBrand} />
              <Text style={styles.sheetLabel}>Category</Text>
              <View style={styles.catRow}>
                {CATEGORIES.map(cat => (
                  <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  card: { flex: 1, margin: 6, borderRadius: 4, overflow: 'hidden', backgroundColor: '#fff', position: 'relative', borderWidth: 3, borderColor: '#1a1a1a', borderBottomWidth: 1, shadowColor: '#000', shadowOffset: { width: 2, height: 3 }, shadowOpacity: 0.15, shadowRadius: 4, elevation: 4 },
